...
THEME = pygame.mixer.Sound('Assets/POL-cyber-factory-short.wav')

HEALTH_FONT = pygame.font.SysFont('edgeofthegalaxyovea6',40)
WINNER_FONT = pygame.font.SysFont('edgeofthegalaxyovea6',100)
...
def red_handle_movement(keys_pressed, red):
    if keys_pressed[pygame.K_LEFT] and red.x - VELOCITY > BORDER.x + BORDER.width: # LEFT: # LEFT
            red.x -= VELOCITY
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LSHIFT and len(yellow_bullets) < NUM_BULLETS:
                    bullet = pygame.Rect(yellow.x + yellow.width,yellow.y + yellow.height//2 - 2,10,5)
                    yellow_bullets.append(bullet)
                    BULLET_FIRE_SOUND.play()
                if event.key == pygame.K_RSHIFT and len(red_bullets) < NUM_BULLETS:
